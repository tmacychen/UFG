...
// Package gl contains an OpenGL implementation of the framework.Driver interface.
package gl
...
func StartDriver(appRoutine func(driver framework.Driver)) {
...
func (d *driver) createDriverEvent(signature interface{}) framework.Event {
	return controller.CreateChanneledEvent(signature, d.pendingDriver)
}

func (d *driver) createAppEvent(signature interface{}) framework.Event {
	return controller.CreateChanneledEvent(signature, d.pendingApp)
}
...
func (d *driver) applicationLoop() {
	...
	println("appalication end")
}

// framework.Driver compliance
...
//	println("send func to pendingApp")
	d.pendingApp <- f
//	println("call end ")
	return true
}
...
//func (d *driver) CreateFont(data []byte, size int) (framework.Font, error) {
//	return NewFont(data, size)
//}

func (d *driver) CreateWindowedViewport(width, height int, name string) framework.Viewport {
	var v *viewport
	d.syncDriver(func() {
...
func (d *driver) CreateFullscreenViewport(width, height int, name string) framework.Viewport {
	var v *viewport
	d.syncDriver(func() {
...
func (d *driver) CreateCanvas(s math.Size) framework.Canvas {
	return newCanvas(s)
}

func (d *driver) CreateTexture(img image.Image, pixelsPerDip float32) framework.Texture {
