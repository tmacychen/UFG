...
type viewport struct {
	...
	onKeyStroke   framework.Event // (framework.KeyStrokeEvent)
	// Broadcasts to driver thread
	onDestroy framework.Event
}
...
func (v *viewport) drawFrameUpdate(ctx *context) {
	...
	ds := &drawState{}
	ctx.blitter.blitRect(ctx, r, tools.White, ds)
}

// framework.viewport compliance
// These methods are all called on the application routine
func (v *viewport) SetCanvas(cc framework.Canvas) {
...
func (v *viewport) OnResize(f func()) framework.EventSubscription {
	return v.onResize.Listen(f)
}

func (v *viewport) OnClose(f func()) framework.EventSubscription {
