...
func pruneDuplicates(p framework.Polygon) framework.Polygon {
	pruned := make(framework.Polygon, 0, len(p))
	last := framework.PolygonVertex{}
	for i, v := range p {
...
func closedPolyToShape(p framework.Polygon, penWidth float32) (fillShape, edgeShape *shape) {
	p = pruneDuplicates(p)
...
func openPolyToShape(p framework.Polygon, penWidth float32) *shape {
	p = pruneDuplicates(p)
	if len(p) < 2 {
		return nil
	}
