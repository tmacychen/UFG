...
func (b *blitter) blitGlyph(ctx *context, tc *textureContext, c tools.Color, srcRect, dstRect math.Rect, ds *drawState) {
	dstRect = dstRect.Offset(ds.OriginPixels)
...
func (b *blitter) blitShape(ctx *context, shape shape, color tools.Color, ds *drawState) {
	b.commitGlyphs(ctx)
...
func (b *blitter) blitRect(ctx *context, dstRect math.Rect, color tools.Color, ds *drawState) {
	b.commitGlyphs(ctx)
	dstRect = dstRect.Offset(ds.OriginPixels)
	dw, dh := ctx.sizePixels.WH()
	mPos := math.CreateMat3(
